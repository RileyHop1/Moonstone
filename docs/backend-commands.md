...
| `compile_project`    | `projectPath`, `mainFile`                      | `CompileOutcome` | Builds a PDF with the bundled Tectonic sidecar; a document that fails to typeset comes back as diagnostics, not an error (see [PDF compilation](pdf-compilation.md)) |
