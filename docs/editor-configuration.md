...
1. **`EditorConfiguration`** — a plain readonly object holding every
   ...
   numbering, diagnostics, references, and the two per-document
   resolvers (`resolveImageSource`, `openLink`) that reconfigure
   through the preview compartment.
...
3. **`editorExtensions(configuration)`** builds what the editor mounts
   with, and **`reconfigurationEffects(previous, next)`** builds what a
   change costs. Both read the same rows, so mount and reconfigure
   cannot drift apart.
...
- **The modal keymap must come first** in the extension list, because
  Vim and Helix register high-precedence keymaps that need to see keys
  before the default bindings. `editorExtensions` enforces that
  ordering rather than leaving it to the order of the `SYNCED` array.
...
3. Populate it where `ProjectPage` builds the configuration.
